...
### 2026-04-03 세션 2 — 채팅 이력 + 로컬 인텐트 + 삼촌이모 + 질문로그
...
| #209 | feat: 가족 초대 역할별 이모지 표시 | ✅ merged |
| #210 | release: 운영 반영 v0403z | ✅ merged |
| #211 | feat: 챗봇 질문 로그 + invite_member 삼촌/이모 enum 추가 | ✅ merged |
| #212 | release: 운영 반영 v0403z2 | ✅ merged |
...
**가족 초대 역할별 이모지 (PR #209)**
- 가족 구성원 초대 성공 카드의 이모지를 역할에 맞게 변경
- 기존: 모든 역할에 👋 → 변경: dad👨 mom👩 grandpa/grandpa_m👴 grandma/grandma_m👵 uncle🧔 aunt👩‍🦱 child🧒 caregiver🤗

**운영 반영 v0403z (PR #210)**
- v0403x~v0403z 전체 운영 반영 (로컬 인텐트 + 삼촌이모 + 역할별 이모지)

**챗봇 질문 로그 (PR #211)**
- 사용자 질문을 Firestore `chatQueries` 컬렉션에 기록 (text, source, uid, role, env 등)
- 3개 입력 경로 모두 로깅: 직접입력(input), 인라인버튼(button), 퀵메뉴(quick_menu)
- admin.html에 챗봇 질문 로그 조회 패널 추가 (환경/소스 필터링, 전체 삭제)
- invite_member 도구 `role` enum에 `uncle`, `aunt` 누락 수정 — 시스템 프롬프트에는 있었으나 도구 정의에 없어서 API가 해당 역할을 선택할 수 없었음

**운영 반영 v0403z2 (PR #212)**
- 질문 로그 + invite_member enum 수정 운영 반영
...
| PR | 설명 | 상태 |
|----|------|------|
| #140 | fix: 로그아웃 시 챗봇 세션 누수 수정 (resetMilyChat) | ✅ merged |
| #141 | feat: 챗봇 폰트 16.5px + 캐릭터 80px + mily.ai SVG 로고 | ✅ merged |
| #142 | feat: 양육자 존댓말/아이 반말 톤 + 인사말 2줄 표시 | ✅ merged |
| #143 | fix: 할인 보상 목표 게이지 할인가 기준 + 마감 안내 | ✅ merged |
| #144 | fix: mily.ai SVG path 로고 (앱 아이콘 폰트 재현) | ✅ merged |
| #145 | fix: mily.ai 로고 y자 하단 잘림 (viewBox 확장) | ✅ merged |
| #146 | feat: 캐릭터·로고 1.35배 확대 + 폰트 증가 | ✅ merged |
| #147 | fix: mily.ai 로고 좌측 정렬 (preserveAspectRatio xMinYMid) | ✅ merged |
| #148 | fix: 챗헤더 gap 줄여 로고 왼쪽 이동 + 다크모드 말풍선 수정 | ✅ merged |
| #149 | release: 운영 반영 v0402z21 | ✅ merged |
| #150 | feat: 챗봇 get_status에 활동별 통계 추가 (횟수/포인트/일수/최근7일/streak) | ✅ merged |
| #151 | feat: 인사말 양육자 존댓말 + 챗봇 용어 마일리지 통일 + P→M | ✅ merged |
| #152 | release: 운영 반영 v0402z23 | ✅ merged |

#### 상세 변경 내용

**챗봇 세션 누수 (PR #140)**
- 문제: logout() 후 재로그인 시 이전 계정의 채팅 데이터가 표시됨
- 수정: logout()에 `_resetMilyChat()` + 패널 닫기 + localStorage 채팅 키 삭제

**mily.ai SVG path 로고 (PR #141, #144~#147)**
- 앱 아이콘의 둥글고 통통한 서체를 SVG stroke path로 직접 재현
- viewBox `-2 6 200 64` + `preserveAspectRatio="xMinYMid meet"` 로 좌측 정렬
- Gemini 스타일 스파클 아이콘 (그라데이션 #818CF8→#C084FC→#F472B6)
- `.ai` 부분은 반투명(rgba 0.5)으로 차별화

**다크모드 챗봇 말풍선 (PR #148)**
- 문제: `--bg2`(#1E293B)가 채팅 배경과 거의 같은색이라 bot 말풍선이 보이지 않음
- 수정: `body.dark .mily-msg.bot { background: var(--card2, #334155) }` 추가
- code/pre 블록도 다크모드에서 `rgba(255,255,255,.08)` 배경 적용

**할인 보상 게이지 (PR #143)**
- 문제: 할인 상품 목표 설정 시 원가 기준 게이지 표시
- 수정: `gSale` 감지 → `gActualPts = Math.round(g.pts * (1 - sale.percent/100))` 적용
- 마감 3일/7일 이내 시 긴급 메시지, 일반 시 할인 기간 안내

**챗봇 활동별 통계 (PR #150)**
- get_status 응답에 `activity_stats`, `recent_7days`, `streak` 추가
- AI가 "제일 잘하는건?" 등 질문에 구체적 횟수/마일리지로 답변 가능
- 시스템 프롬프트에 통계 활용 가이드 추가

**인사말 양육자 존댓말 (PR #151)**
- 홈 화면 하드코딩 인사말 12종: 양육자=존댓말, 아이=반말 분기
- AI 인사말 프롬프트: `toneGuide` 변수로 존댓말/반말 분기
- 힌트 문구: "저를 눌러서 대화해보세요!" (양육자) vs "나를 눌러서 대화해봐!" (아이)

**용어 통일: 포인트→마일리지, P→M (PR #151)**
- 시스템 프롬프트: [용어] 규칙 추가 — "포인트" 절대 금지, "마일리지" 또는 "M" 사용
- tool result 메시지: "+20P 적립" → "+20M 적립"
- 탭별 인사이트 버블: 모든 P→M 변환
- 육아코칭 근거: 포인트→마일리지
- tool description: "마일리지 포인트" → "마일리지"

**톤 & 인사말 (PR #142)**
- 양육자: 존댓말 (~요, ~해요, ~드릴게요)
- 아이: 반말 (~야, ~해, ~거야) 친근하게
- 홈 인사말: `-webkit-line-clamp: 3` 으로 2줄+ 표시
...
| #120 | fix: 챗봇 모델 ID 수정 + 시스템 프롬프트 최적화 + 에러 안전성 | ✅ 개발기 적용 |
| #121 | feat: AI 인사말 기능 (6시간 캐시, 백그라운드 API 호출) | ✅ 개발기 적용 |
| #122 | feat: 챗봇 중복 확인 규칙 (get_status 선조회) | ✅ 개발기 적용 |
| #123 | feat: 다자녀 대상 확인 규칙 (모든 조치 전 자녀 확인) | ✅ 개발기 적용 |
| #124 | fix: iPad/데스크톱 채팅 카드 팝업 레이아웃 (400×680px) | ✅ 개발기 적용 |
| #125 | fix: visualViewport 핸들러 모바일(700px 미만) 전용 가드 | ✅ 개발기 적용 |
| #126 | fix: 패드 채팅 backdrop 제거 | ✅ 개발기 적용 |
| #128 | release: 운영기 반영 v0402z6 | ✅ 운영기 배포 |

### 2026-04-02 세션 (챗봇 안정화 + AI인사말 + 운영 배포)

**주요 변경 (v0402z6)**
- 챗봇 모델 ID 수정: `claude-haiku-4-5-20241022` (존재하지 않음) → `claude-haiku-4-5-20251001` (PR #120)
- 시스템 프롬프트 최적화: 95줄 → ~15줄, 토큰 절약 (PR #120)
- `_buildSystemPrompt()`/`_buildTools()`를 try 블록 안으로 이동 — 에러 시 타이핑 인디케이터 무한 대기 방지 (PR #120)
- AI 인사말: 홈 화면에서 6시간마다 Claude API로 상황별 맞춤 인사 생성, localStorage 캐시 (PR #121)
- 챗봇 중복 확인 규칙: 활동/보상 추가 전 `get_status`로 기존 목록 조회, 유사 항목 있으면 안내 (PR #122)
- 다자녀 대상 확인: 모든 조치(추가/수정/삭제/수행/취소/교환) 전 "어떤 자녀?" 확인 (PR #123)
- iPad/데스크톱 채팅: 전체화면 → 카드 팝업(400×680px, border-radius:24px) (PR #124)
- 패드 채팅 backdrop 제거: 배경 가리지 않고 팝업만 표시 (PR #126)
- visualViewport 핸들러: 모바일(width<700px) 전용으로 제한 — 패드에서 레이아웃 깨짐 방지 (PR #125)
...
### 2026-04-12 세션 — UI 통일 + 로딩 화면 태그라인 + 뱃지 수령 아이콘 + 곤충 SVG 수정

**적용 범위: 개발기 (v0410b~m) + 운영기 (PR #306~321)**

#### 주요 변경사항

1. **스플래시 화면 로고 교체 (v0410b, PR #306)**
   - `#mily-splash`의 mily 캐릭터를 mile.ly 로고로 교체
   - 글래스모피즘 + pulse 애니메이션

2. **편집 버튼 스타일 개선 (v0410c~i, PR #307~314)**
   - 원형 버튼 → 둥근 사각형 + 연필 SVG 아이콘
   - 여러 차례 반복 수정 후 최종적으로 홈탭 이력보기 버튼과 동일한 CSS로 통일
   - `padding:5px 11px; border:1.5px solid var(--bdr2); border-radius:14px; background:var(--bg2); color:var(--tx2); font-size:12px`
   - JS 토글(편집↔완료) 시에도 동일 형태 유지

3. **섹션 제목 아이콘-텍스트 정렬 (v0410e, PR #309)**
   - 모든활동/교환가능한보상 섹션의 `.sec-ico`와 텍스트 수직 정렬 수정

4. **로딩 화면 리디자인 (v0410f~h, PR #310~312)**
   - `showLoading()` 함수의 mily 캐릭터를 mile.ly 브랜드 로고 SVG로 교체
   - 기존 앱 SVG 폰트 path 그대로 사용 (viewBox="34 248 3020 972")
   - 로고 크기 36px→48px 확대
   - 태그라인 "성장하는 우리 아이의 첫 마일리지" 추가

5. **홈 인사말 캐릭터 원복 (v0410f, PR #310)**
   - v0410d에서 잘못 변경한 홈 인사말 mily 캐릭터를 원래대로 복원
   - `_milySVG()` 함수로 렌더링하는 것이 올바른 동작

6. **뱃지 목록 수령 상태 아이콘 (v0410j~l, PR #315~319)**
   - 뱃지 보기 화면(`renderBadgeViewGrid`)에 수령 상태 아이콘 표시
   - 수령완료: 우상단 초록 원 + 흰색 체크마크 (22×22px, `#10B981`)
   - 미수령(획득): 우상단 노란 원 + 선물 아이콘 + `bdg-pulse` 애니메이션 (`#F59E0B`)
   - 미획득: 아이콘 없음
   - `.badge-item`에 `position:relative` 추가
   - **주의**: v0410j에서 `renderBadges()`만 수정하고 실제 화면인 `renderBadgeViewGrid()`를 누락 → v0410l에서 수정

7. **뱃지 수령 후 즉시 갱신 (v0410m, PR #320~321)**
   - `renderAll()`에 `renderBadgeViewGrid()`와 `renderBadgeViewHistory()`가 누락되어 있었음
   - 수령 후 목록 아이콘이 즉시 갱신되지 않던 문제 수정

8. **메뚜기 뒷다리 SVG 수정 (v0410k, PR #317)**
   - 뒷다리 무릎 위치 `(3,5)` → `(5,5)`: 대칭 'ㅅ' 형태
   - 발끝 위치 `y=18` → `y=16`: 앞다리와 동일 높이
   - 불필요한 발 연장 `L4 21` 제거

9. **브랜드 가이드라인 CLAUDE.md 추가 (v0410f)**
   - mile.ly 로고 텍스트 SVG 경로 절대 변경 금지 규칙 추가
   - 로딩 화면 = `showLoading()` → `#fb-loading` 명시
   - 홈 인사말 mily 캐릭터는 `_milySVG()` 유지 규칙 명시

#### 트러블슈팅 기록

##### 스플래시 vs 로딩 화면 혼동 (재발)
- **증상**: 사용자가 로딩 화면 수정을 요청했으나 스플래시(`#mily-splash`)만 수정됨
- **원인**: 앱에 두 개의 로딩 관련 화면이 존재하는 것을 혼동
  - `#mily-splash`: 앱 초기 로드 스플래시 (사용자가 거의 못 봄)
  - `showLoading()` → `#fb-loading`: 실제 데이터 로딩 화면 (사용자가 매번 봄)
- **해결**: 사용자 스크린샷으로 `showLoading()` 함수가 올바른 대상임을 확인
- **교훈**: 로딩 화면 요청 시 반드시 사용자에게 어느 화면인지 확인

##### 홈 인사말 캐릭터 잘못 변경
- **증상**: 사용자가 요청하지 않은 홈 인사말 영역의 mily 캐릭터가 로고로 변경됨
- **원인**: 로딩 화면과 홈 인사말을 혼동하여 둘 다 변경
- **해결**: v0410f에서 `charEl.innerHTML=_milySVG(charCfg)`로 원복

##### 뱃지 렌더링 함수 2개 혼동
- **증상**: 뱃지 수령 아이콘이 안 보임
- **원인**: `renderBadges()`(나의 메뉴 내 뱃지 섹션)만 수정하고, 실제 사용자가 보는 `renderBadgeViewGrid()`(뱃지 보기 전용 화면)를 누락
- **해결**: v0410l에서 `renderBadgeViewGrid()`에도 동일 아이콘 추가
- **교훈**: 뱃지 관련 수정 시 두 렌더링 함수 모두 확인:
  - `renderBadges()` — 나의 메뉴 내 뱃지 요약
  - `renderBadgeViewGrid()` — 뱃지 보기 전용 화면 (사용자가 주로 보는 화면)

##### renderAll() 뱃지 뷰 누락
- **증상**: 뱃지 마일리지 수령 후 목록이 즉시 갱신되지 않음
- **원인**: `renderAll()`에 `renderBadgeViewGrid()`와 `renderBadgeViewHistory()`가 포함되어 있지 않았음
- **해결**: v0410m에서 `renderAll()`에 두 함수 추가

#### PR/커밋 이력

| 버전 | PR | 주요 내용 |
|------|-----|----------|
| v0410b | #306 | 스플래시 로고 교체 |
| v0410c | #307 | 편집 버튼 둥근 사각형 |
| v0410d | #308 | (실수) 홈 인사말 로고 교체 |
| v0410e | #309 | 섹션 제목 정렬 |
| v0410f | #310 | 로딩 화면 로고 + 홈 인사말 원복 + 편집 버튼 개선 |
| v0410g | #311 | 편집 버튼 패딩/모서리 미세 조정 |
| v0410h | #312 | 로딩 화면 태그라인 + 로고 확대 |
| v0410i | #313, #314(운영) | 편집 버튼 이력보기 스타일 통일 |
| v0410j | #315, #316(운영) | 뱃지 수령 아이콘 (renderBadges만 — 불완전) |
| v0410k | #317 | 메뚜기 뒷다리 ㅅ 대칭 |
| v0410l | #318, #319(운영) | 뱃지 수령 아이콘 (renderBadgeViewGrid 수정) |
| v0410m | #320, #321(운영) | 뱃지 수령 후 즉시 갱신 |

### 2026-04-13 세션 — 보안 강화 + 곤충 SVG + 다크모드 + 소개 페이지

#### 주요 변경사항

| 버전 | PR | 주요 내용 |
|------|-----|----------|
| v0413a | #322 | 회원가입 역할 선택 아이콘 8종 리디자인 |
| v0413b→c | #323, #324 | 역할 아이콘 순서 수정 + 아빠 눈썹 추가 |
| v0413c | #324 | 챗봇 대화기록 사용자별 분리 + 가족 전체 삭제 기능 |
| v0413d | #325 | 소개 페이지 실제 앱 스크린샷 교체 (8장, 다크/라이트 자동 전환) |
| v0413e | #326 | 역할 미선택 시 건너뛰기 방지 |
| v0413f | #328 | 가족 삭제를 시스템 관리자 페이지(admin.html)로 이동 |
| v0413g | #330(운영) | 운영 반영 (v0413f까지) |
| v0413j | #333, #334(운영) | 곤충 SVG 아이콘 교체 + 다크모드 glow 효과 + 운영 반영 |

#### 곤충 선택 SVG 아이콘 교체 (R-028~R-029)
- 나의 메뉴 > 챌린지 > 곤충 선택에서 이모지(🐞🐜🦗 등)를 SVG 아이콘으로 교체
- `_renderBugPicker()` 함수에서 `window.BUG_TYPES[id].svg`를 직접 렌더링
- 랜덤 옵션은 주사위 패턴 SVG 사용

#### 다크모드 곤충 가시성 (R-029 보완)
- **시도 1** (기각): 밝은 배경 → 사용자 "별로야"
- **시도 2** (기각): SVG stroke/fill 색상 반전 → "정말 이상해"
- **시도 3** (채택): `drop-shadow` glow 효과
  ```css
  body.dark .bug-pick .bp-svg{filter:drop-shadow(0 0 3px rgba(255,255,255,.7)) drop-shadow(0 0 6px rgba(255,255,255,.3))}
  ```

#### 가족 삭제 시스템 관리자 이동 (R-034)
- 앱 내 관리자 설정(adm-p4)에서 `deleteFamilyAll()` 제거
- `dev/admin.html`의 가족상세 페이지에 "위험 구역" 섹션 추가
- CLAUDE.md에 관리자 페이지 구분 지침 추가 (앱 내 관리자 vs 시스템 관리자)

### 2026-04-14 세션 — 디자인 토큰 표준화 (Phase 1+2)

#### 배경
`DESIGN_REVIEW.md` 디자인 리뷰에서 다음 문제가 발견됨:
- 427개 고유 색상 (권장 20~40개)
- 15종 이상 border-radius (권장 3~5종)
- 16단계 font-size (권장 5~7단계)
- 92회 그라디언트 사용

#### Phase 1: 색상 팔레트 CSS 변수화

**추가된 CSS 변수 (12개):**
| 변수 | 값 | 설명 |
|------|-----|------|
| `--or-dk` | #4F46E5 | 진한 인디고 |
| `--or-bright` | #818CF8 | 밝은 인디고 |
| `--or-pale` | #A5B4FC | 연한 인디고 |
| `--gn-bright` | #34D399 | 밝은 초록 |
| `--rd-dk` | #DC2626 | 진한 빨강 |
| `--pu-bright` | #A78BFA | 밝은 보라 |
| `--pu-dk` | #5B4FC4 | 진한 보라 |
| `--yw-dk` | #D97706 | 진한 노랑/호박 |
| `--yw-bright` | #FBBF24 | 밝은 노랑 |
| `--rxs` | 8px | border-radius 극소 |
| `--rmd` | 16px | border-radius 중간 |
| `--rfull` | 999px | border-radius 원형 |

**교체 내역:** 하드코딩 색상 349개 → `var()` 참조로 교체

**교체 대상 색상 (주요):**
| 색상 | 교체 수 | 변수 |
|------|---------|------|
| #6366F1 | 96x | var(--or) |
| #818CF8 | 48x | var(--or-bright) |
| #EF4444 | 33x | var(--rd) |
| #64748B | 27x | var(--tx2) |
| #94A3B8 | 23x | var(--tx3) |
| #EEF2FF | 15x | var(--or-lt) |
| #10B981 | 13x | var(--gn) |

#### Phase 2: 디자인 토큰 표준화

**border-radius 통합 (15종 → 7단계):**
| 변수 | 값 | 병합 대상 | 교체 수 |
|------|-----|----------|---------|
| `--rxs` | 8px | 6~9px | — |
| `--rsm` | 12px | 10~14px | — |
...
**font-size 통합 (16단계 → 9단계):**
| 변수 | 값 | 병합 대상 | 교체 수 |
|------|-----|----------|---------|
| `--fs-xs` | 10px | 8~10px | — |
| `--fs-sm` | 11px | 11px | — |
| `--fs-base` | 13px | 12~13px | — |
| `--fs-md` | 14px | 14~15px | — |
| `--fs-lg` | 16px | 16~17px | — |
| `--fs-xl` | 18px | 18px | — |
| `--fs-2xl` | 22px | 20~22px | — |
...
> box-shadow는 탭별 고유 글로우 색상 등 맥락 의존적이라 변수 정의만 추가. 자동 교체는 보류.

#### 결과 요약
| 지표 | Before | After |
|------|--------|-------|
| CSS 변수 사용 | 742회 | 1,752회 (+136%) |
| 하드코딩 색상 | 693개 | 344개 (-50%) |
| border-radius 종류 | 15+ | 7단계 |
| font-size 종류 | 16+ | 9단계 |
...
| 버전 | PR | 주요 내용 |
|------|-----|----------|
| v0417d | PR #401(dev), #402(prod) | 7개 배열 + memberData 서브컬렉션 분리, 자동 마이그레이션, echo detection 수정 |

---

### 12.24 사고 5: v0417e 통합모드 제거 배포 후 전면 장애 (2026-04-17)

#### 증상
- 운영기: 로그인 시 "패스워드가 틀립니다" 에러
- 개발기: 로그인 후 0 마일리지, 모든 버튼 무반응

#### 근본 원인 — 함수 삭제 후 window export에 참조 잔존
v0417e에서 `selectMode()`, `createStep4Next()` 함수 본체를 삭제했으나, window export 목록(24641, 24649줄)에 해당 심볼이 남아 있었음:
```javascript
// 함수는 삭제했는데 이 줄이 남아있음 → ReferenceError 발생
startCreateFamily, ..., createStep4Next, ...  // ← 존재하지 않는 함수
selectMode,                                    // ← 존재하지 않는 함수
```
25,000줄 단일 HTML 파일에서 ReferenceError 하나가 전체 스크립트 실행을 중단 → `doLogin`, 버튼 핸들러 등 모든 함수 미정의.

#### 2차 원인 — `_createState.mode` 속성 삭제 후 참조 잔존
`_createState` 객체에서 `mode: 'independent'` 속성을 제거했지만, 코드 3곳(6397, 6609, 6851줄)에서 `_createState.mode`를 여전히 참조. 첫 번째 에러로 실행이 안 됐기에 실제 피해 없었으나, 없었더라도 새 가족 생성 시 `mode: undefined` 저장 문제 발생했을 것.

#### 3차 피해 — 롤백 시 데이터 접근 불가 (사고 2 패턴 반복)
v0417e 긴급 롤백 → v0417c 복원 → v0417d에서 이미 서브컬렉션으로 마이그레이션된 데이터를 v0417c가 읽지 못함 → 전 구성원 0 마일리지 표시. 데이터 구조 변경(v0417d)과 코드 리팩토링(v0417e)을 같은 날 배포한 것이 복합 사고의 원인.

#### 복구 과정
1. PR #405: v0417c로 긴급 롤백 (로그인/버튼 문제 해소, but 데이터 0 표시)
2. PR #406: v0417d 복원 (서브컬렉션 읽기 가능한 코드 재배포 → 데이터 정상화)

#### 교훈
1. 함수/변수 삭제 전 반드시 `grep -n "함수명" dev/index.html`로 모든 참조 확인
2. `replace_all`로 조건문 패턴 일괄 변경 금지 — 개별 확인 필수
3. 데이터 구조 변경과 코드 리팩토링은 동시 배포 금지 — 최소 며칠 간격
4. 배포 후 브라우저 콘솔 확인 필수 — ReferenceError 한번이면 즉시 발견 가능했음
