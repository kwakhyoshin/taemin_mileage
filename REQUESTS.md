...
| R-083 | 2026-04-21 | [진단/사고 11] v0421a/b/c 세 차례 핫픽스 후에도 아이폰 PWA에서 로그인 실패 지속(맥 크롬·아이패드 PWA 정상). 디바이스 콘솔 접근 불가 상태 — 원격 텔레메트리 수집 채널 필요. 사용자 요청: "진단로그 복사보다는 보내기 버튼을 만들어서 그걸 누르면 디비 테이블에 저장되고 너가 그걸 직접 api로 조회해서 보면 되지 않을까? 그리고 해당 데이터는 시스템관리자페이지(앱에있는 관리자 기능말고 admin.html)에 로그들을 확인 할수 있는 기능도 만들고." | v0421d | — | 1) 로그인 화면 "🩺 진단 실행" 링크 — `_runLoginDiagnostic`가 deviceHint·standalone·localStorage·IndexedDB·CacheStorage 인벤토리·Firebase Auth snapshot·`_id_registry` `getDocFromServer` 결과·family 문서 fetch·Firestore REST 네트워크 도달성을 수집. 2) "📤 진단 전송" 버튼 → Firestore `diagLogs` 컬렉션 `addDoc`. 비밀번호는 `passwordLen`만, 시도 ID는 저장. 3) `admin.html` 사이드바 "로그인 진단 로그" 패널 — `diagLogs` orderBy(`clientTime` desc) limit 100 + JSON 상세 + 전체 삭제. 4) Firestore 규칙: `match /diagLogs/{docId} { allow read, write: if true; }` (emailVerify와 동일 공개 패턴). dev_guide 사고 11 기록. |
