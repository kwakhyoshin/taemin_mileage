...
| R-086 | 2026-04-22 | [버그] Android 갤럭시 홈탭 느린 스크롤 시 mily 인사말 영역이 사라지지 않고 헤더 축소가 멈춤. 빠른 스크롤은 정상. 아이폰은 정상 — Chromium scroll anchoring 이 hero.compact 전환으로 인한 높이 변화를 상쇄하려 scrollY 를 역보정 → effScrollY 가 thresholdOff(40) 아래로 떨어져 compact 제거 → 다시 threshold(60) 넘어 add → 무한 토글 피드백 루프 | v0422a | — | html/body/#s-home/.scr 에 `overflow-anchor:none` 적용. Chromium scroll anchoring 완전 비활성화로 hero 높이 변화가 scrollY 에 영향 못 주도록 차단. dev_guide 섹션 "Android scroll anchoring 느린 스크롤 버그" 기록. |
