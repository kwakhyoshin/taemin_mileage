...
| R-082 | 2026-04-21 | [버그/사고 10] v0421b "로컬 상태 초기화" 후 로그인 시 홈 화면에 0 마일리지/빈 활동기록 일시 표시(앱 재시작 시 정상 복원) — Firebase Auth 세션이 localStorage에 잔존해 리로드 직후 자동 로그인이 빈 state 위에 홈을 렌더링 + Tier 3 로그인 경로가 서브컬렉션(log/memberData 등)을 동기 로드하지 않음 | v0421c | v0421c | Part A: `_authHardReset`에 `signOut(fbAuth)` 선행 + localStorage `firebase:` prefix 키 삭제 + `_milely_force_login='1'` 마커. Part B: `_setupAuthListener`와 `onAuthStateChanged` 콜백 양쪽에 force-login 가드 — 마커 감지 시 signOut 강제, 자동 로그인 차단. Part C: `doLogin` Tier 3 matched 블록에 `familyData._subCollMigrated===true` 시 `_readSubCollections`/`_readMemberSubCollections` Promise.all 동기 로드 + `_setupSubColListeners` 활성화 — `_applyRemoteSnap` 경로와 동일한 서브컬렉션 처리 패턴을 Tier 3에도 적용. dev_guide 사고 10 기록. |
