...
| R-034 | 2026-04-13 | 가족 전체 삭제를 시스템 관리자 페이지(admin.html)로 이동 | v0413f | 개발완료 | 앱 내 관리자에서 제거 → admin.html 가족상세에 추가. CLAUDE.md에 관리자 구분 지침 추가 |
| R-035 | 2026-04-13 | 운영기 소개페이지 이미지 경로 수정 (ob-*.png → dev/ob-*.png) | v0413f | 개발완료 | index.html이 루트에 있어 dev/ 폴더 이미지 참조 불가 문제 |
