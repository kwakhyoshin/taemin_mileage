...
- AI `reject` → `status: rejected`, 마일리지 적립 X. **활동 기록은 남기고** 카드에 `🤖 반려` 칩 + 사유 표시. 카운트 +1.
...
6. 결과 분기:
   ...
   - **reject**: 마일리지 적립 X. 친근한 사유를 모달에 표시 + "다시 도전" 또는 "닫기" 버튼. **활동 기록은 `rejected` 상태로 남고** 활동 카드에 `🤖 반려` 칩과 사유가 표시됨. **카운트 +1** (무한 재시도 방지). 부모는 사후에 칩을 눌러 사유를 보고 필요시 통과로 오버라이드 가능
...
**칩 종류**:
- `🤖 통과` (초록) — AI 통과
- `🤖 반려` (빨강) — AI 반려. 사유는 미니 코멘트 줄에 표시 + 칩 클릭 시 모달에 전문
...
**미니 코멘트 줄**:
- 통과·반려 모두 활동 시간 옆에 코멘트 첫 20자를 작은 글씨로 표시. 통과는 회색, 반려는 살짝 붉은 톤으로 구분.
- 부모가 칩을 누르지 않아도 흐름 따라가며 사유를 볼 수 있음.
...
| reject 처리 | 마일리지 적립 X, **활동 기록은 `rejected` 상태로 보존** + 사유 표시. 카운트 +1. 부모 사후 오버라이드 가능 |
