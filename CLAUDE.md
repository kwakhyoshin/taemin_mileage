...
### 사고 5: 통합모드 제거(v0417e) 배포 후 로그인 실패·데이터 소실 (2026-04-17)
- **원인**: v0417e에서 통합모드 코드를 `replace_all`로 제거하면서 로그인·버튼 동작 관련 코드까지 영향. 정확한 원인은 `getFamilyMode()==='independent'&&` 패턴 일괄 제거 시 조건문 구조 손상 가능성
- **2차 피해**: 긴급 롤백으로 v0417c 복원 → v0417c는 서브컬렉션 미지원 → 이미 서브컬렉션으로 마이그레이션된 데이터를 읽지 못해 0 마일리지 표시 (사고 2 패턴 반복)
- **복구**: v0417d(서브컬렉션 지원 코드) 재배포(PR #406)로 데이터 정상 복원
- **교훈**:
  1. `replace_all`로 조건문 패턴을 일괄 제거할 때는 각 변경 지점의 전후 맥락 확인 필수
  2. 데이터 구조를 변경하는 코드(서브컬렉션 분할)와 기능 제거 코드는 절대 동시에 배포하지 말 것 — 롤백 시 데이터 접근 불가
  3. 서브컬렉션 마이그레이션 이후에는 마이그레이션 전 코드로 롤백 불가 — 롤백 대상은 반드시 서브컬렉션을 읽을 수 있는 버전이어야 함
